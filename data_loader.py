...
class MVTecDRAEMTrainDataset(Dataset):
    ...
    def __init__(self, root_dir, anomaly_source_path, resize_shape=None):
        ...
        foreground_paths = os.path.join('',self.obj_name , 'foreground/' )  # foreground dir
    ...
    def transform_image(self, image_path, anomaly_source_path, foreground_path):
        ...
        return image, augmented_image, anomaly_mask, has_anomaly, foreground_image
    

    def __getitem__(self, idx):
        idx = torch.randint(0, len(self.image_paths), (1,)).item()
        anomaly_source_idx = torch.randint(0, len(self.anomaly_source_paths), (1,)).item()
        image, augmented_image, anomaly_mask, has_anomaly, foreground_image = self.transform_image(self.image_paths[idx],
                                                                           self.anomaly_source_paths[anomaly_source_idx],self.image_foreground_paths[idx])
        ...
        sample = {'image': image, "anomaly_mask": anomaly_mask,
                  'augmented_image': augmented_image, 'has_anomaly': has_anomaly, 'idx': idx, 'foreground_image': foreground_image, 'foreground_image_new':foreground_image_2_}
